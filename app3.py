...
with tab_export:
    ...
    if final_selection:
        ...
        if not bulk_activity_df.empty:
            ...
            # Only filter if we have a date column AND a valid range
            if date_col_found and len(date_range) == 2:
                # Convert dataframe column to just DATE objects (removes timezone/time mismatch issues)
            ...
            buffer = io.BytesIO()
            # --- FIX 2: Removed engine='xlsxwriter' to allow pandas to auto-select default (openpyxl) ---
